...
public class Add extends HttpServlet{
	...
	static ArrayList<Test_pojo> data ;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String line = null;
		Test_pojo invoice = new Test_pojo();
		...
		try {
			...
			invoice.setBill_id((long)Double.parseDouble(jsonObject.getString("invoice_id")));
		...
		try {
			...
			String sql = "INSERT INTO invoice_details (cust_number,name_customer,bill_id,total_open_amount,due_in_date,doc_id,notes) VALUES (?,?,?,?,?,?,?)";
			pst = conn.prepareStatement(sql);
			
			pst.setString(1, invoice.getCust_number());
			pst.setString(2, invoice.getName_customer());
			Long id = invoice.getBill_id();
			if(id != null) {
				pst.setLong(3, id);
			}else {
