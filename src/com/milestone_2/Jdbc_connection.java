...
public class Jdbc_connection {
	...
	public static void main(String[] args) {
		...
		Data obj = new Data();
		ArrayList<Test_pojo> arr = obj.data_read();
		int batchSize = 1000;
		...
		try {
			...
			String sql = "INSERT INTO invoice_details (business_code, cust_number, name_customer, clear_date, business_year, "
					+ "doc_id, posting_date, document_create_date, document_create_date_1, due_in_date, bill_currency, document_type,"
					+ " posting_id, area_business, total_open_amount, baseline_create_date, cust_payment_terms, bill_id, isOpen)"
					+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			for(int i = 0; i < arr.size(); i++) {
				...
				pst.setString(11, arr.get(i).getBill_currency());
